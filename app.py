...
# --- 🔑 ضع مفتاحك في السطر 24 بالأسفل ---
API_KEY = "AIzaSyD9WBNpqzGhS47RfFrw0YqPb40TbB8dX9M"
# ---------------------------------------

def get_ai_response(prompt):
    if "ضع_مفتاحك" in API_KEY or not API_KEY:
        return "⚠️ يرجى وضع مفتاح API Key في السطر 24 أولاً."
    try:
        genai.configure(api_key=API_KEY.strip())
        # استخدام الموديل الأحدث لتجنب خطأ 404 الظاهر في الصور
        model = genai.GenerativeModel('gemini-1.5-flash')
        response = model.generate_content(prompt)
        return response.text
    except Exception as e:
        return f"❌ حدث خطأ في الاتصال. يرجى التأكد من صلاحية المفتاح. (السبب: {str(e)})"
...
if menu == "📊 حاسبة الصحة":
    ...
    if st.button("إجراء التحليل"):
        ...
        with st.spinner("جاري استشارة الذكاء الاصطناعي..."):
            res = get_ai_response(f"أنا {gender}، عمري {age}، وحالتي {status}. كتلة جسمي {bmi:.1f}. اعطني نصيحة جزائرية.")
            st.info(res)
